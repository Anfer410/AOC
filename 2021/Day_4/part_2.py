...
def bingo():
    ...
    for number in numbers:
        for board in reversed(range(len(boards))):
            find_number_on_board(boards[board], number)
            
            if check_for_win_cols(boards[board]) == 'Won' or check_for_win_rows(boards[board]) == 'Won':
                print('STOP', number)
                print("Won:", boards[board])
                won_board = boards[board]
                won_number = number
                del boards[board]

    return won_board, won_number
                
            
def won_calculation(matrix):
    mtrxrows = len(matrix)
...
def main():
    ...
    won_board, won_number = bingo()

    calc = won_calculation(won_board)
    print("Result: ", calc * won_number)
